...
baseUrl},UrlParser.prototype.checkCORS=function(parsedUrl){if(parsedUrl=parsedUrl||new UrlParser(window.location),parsedUrl instanceof UrlParser)return this.get("protocol")===parsedUrl.get("protocol")&&this.get("hostname")===parsedUrl.get("hostname")&&this.get("port")===parsedUrl.get("port");throw new Error("parsedUrl parameter must be an instanceof UrlParser")},UrlParser}),define("serviceApi/ServiceApi",["jquery","urlParser","iframeResizer"],function($,UrlParser,iframeResizer){function ServiceApi(baseUrl,parameters,serviceCallId,stateStorage,userService){this.baseUrl=baseUrl,this.parameters=parameters,this.connected=!1,this.serviceCallId=serviceCallId,this.state=stateStorage,
...
},ServiceApi.prototype.onFinish=function(callback){this.onFinishCallback=callback},ServiceApi.prototype.onKill=function(callback){this.onKillCallback=callback},ServiceApi.prototype.kill=function(callback){"function"==typeof this.onKillCallback?this.onKillCallback(callback):callback(0)},ServiceApi.prototype.finish=function(valueArray){"function"==typeof this.onFinishCallback&&this.onFinishCallback(valueArray)},ServiceApi}),define("serviceApi/StateStorage",["jquery"],function($){"use strict";function StateStorage(state,submitUrl){this.state=state,this.submitUrl=submitUrl}return StateStorage.prototype.get=function(callback){return"function"==typeof callback&&callback(this.state),
this.state},StateStorage.prototype.set=function(state,callback){state===this.state?"function"==typeof callback&&callback():(this.state=state,$.ajax({url:this.submitUrl,data:{state:state},type:"post",dataType:"json",success:"function"==typeof callback?callback:null}))},StateStorage}),define("serviceApi/UserInfoService",["jquery"],function($){function UserInfoService(requestUrl,data){this.data=data,this.requestUrl=requestUrl}return UserInfoService.prototype.get=function(property,callback){this.data.hasOwnProperty(property)?"function"==typeof callback&&callback(this.data[property]):$.ajax({url:this.requestUrl,data:{property:property},type:"post",dataType:"json",
success:function(service,callback){return function(r){for(key in r.data)service.data[key]=r.data[key];"function"==typeof callback&&callback(service.data[property])}}(this,callback)})},UserInfoService}),define("core/format",["lodash"],function(_){"use strict";var pattern=/(%[sdj])/g;return function(message){var replacements=Array.prototype.slice.call(arguments,1);return _.reduce(message.match(pattern),function(acc,match,index){var replacement="";if(replacements[index]){switch(match){case"%d":replacement=Number(replacements[index]);break;case"%j":try{replacement=JSON.stringify(replacements[index]).replace(/"/g,"")}catch(e){}break;default:replacement=replacements[index];
}message=message.replace(match,replacement)}return message},message)}}),define("i18n",["lodash","json!i18ntr/messages.json","context","core/format"],function(_,i18nTr,context,format){"use strict";var translations=i18nTr.translations,__=function(message){var localized=translations[message]?translations[message]:message;return arguments.length>1&&(arguments[0]=localized,localized=format.apply(null,arguments)),localized};return __}),define("util/regexEscape",[],function(){"use strict";return function(s){return s.replace(/[-\/\\^$*+?.()|[\]{}]/g,"\\$&")}}),define("util/wrapLongWords",["util/regexEscape"],function(regexEscape){"use strict";function wrapLongWords(str,threshold){
str=str.replace(/([\w])</g,"$1 <");for(var cut,chunkExp=new RegExp(".{1,"+threshold+"}","g"),longWords=str.match(new RegExp("[\\S]{"+threshold+",}","g"))||[],i=longWords.length;i--;)cut=longWords[i].match(chunkExp).join(" "),str=str.replace(new RegExp(regexEscape(longWords[i]),"g"),cut);return str}return wrapLongWords}),define("util/encode",[],function(){"use strict";var _reQuot=/"/g,_reApos=/'/g,encodeHTML=function(html){return document.createElement("a").appendChild(document.createTextNode(html)).parentNode.innerHTML},encodeAttribute=function(html){return encodeHTML(html).replace(_reQuot,"&quot;").replace(_reApos,"&apos;")};return{html:encodeHTML,attribute:encodeAttribute
...
dElem.className+=" fullscreen",$dialog.modal("close"))):void $dialog.modal("close")},init=function(){$body=$(document.body),document.addEventListener(fs.fullscreenchange,function(){fs.fullScreen()||(dElem.className=dElem.className.replace(/\bfullscreen\b/,""),$dialog.modal("open"))}),modal($body),$dialog=$(dialogTpl({fsSupported:fs.isSupported,launchButton:0===navigator.platform.toLowerCase().indexOf("Mac")?"Ctrl+⌘+F":"F11"})),$dialog[0].querySelector("button").addEventListener("click",function(e){handleUserInput(e)}),document.addEventListener("keydown",function(e){fs.fullScreen()||handleUserInput(e)}),$dialog.on("opened.modal",function(){clearInterval(fs.changeInterval);
}),$dialog.on("closed.modal",function(){fs.awaitFsChange()}),$body.append($dialog),$dialog.modal({width:500,disableClosing:!0,startClosed:!0}),!1==(screen.availHeight||screen.height-30)<=window.innerHeight&&$dialog.modal("open")};return{init:init}}),define("layout/loading-bar",["jquery"],function($){"use strict";function getHeaderHeight(headerElements){var $element,headerHeight=0;for($element in headerElements)headerElements[$element].length&&headerElements[$element].is(":visible")&&(headerHeight+=headerElements[$element].outerHeight());return headerHeight}var $loadingBar=$(".loading-bar"),originalHeight=$loadingBar.height(),$win=$(window),$doc=$(document),$contentWrap=$(".content-wrap"),headerElements={
$versionWarning:$contentWrap.find(".version-warning"),$header:$contentWrap.find("header:first()")},headerHeight=getHeaderHeight(headerElements);return $win.on("scroll.loadingbar",function(){$loadingBar.hasClass("loading")&&(headerHeight=getHeaderHeight(headerElements),headerHeight<=$win.scrollTop()?$loadingBar.addClass("fixed"):$loadingBar.removeClass("fixed"),$loadingBar.height($doc.height()))}),{start:function(){$loadingBar.hasClass("loading")&&$loadingBar.stop(),$loadingBar.addClass("loading"),$win.trigger("scroll.loadingbar")},stop:function(){$loadingBar.removeClass("loading fixed").height(originalHeight)}}}),define("taoDelivery/controller/runtime/deliveryExecution",["jquery","module","iframeResizer","context","serviceApi/ServiceApi","serviceApi/StateStorage","serviceApi/UserInfoService","taoDelivery/controller/runtime/fullScreen","layout/loading-bar"],function($,module,iframeResizer,context,ServiceApi,StateStorage,UserInfoService,fullScreen,loadingBar){
"use strict";function resizeMainFrame(){var height=$(window).outerHeight()-$headerHeight-$footerHeight;$frameContainer.height(height),$frame.height(height)}var $frameContainer,$frame,$headerHeight,$footerHeight,serviceApi,config=module.config();config.deliveryServerConfig.requireFullScreen&&fullScreen.init(),$frameContainer=$("#outer-delivery-iframe-container"),$frame=$frameContainer.find("iframe"),$headerHeight=$("body > .content-wrap > header").outerHeight()||0,$footerHeight=$("body > footer").outerHeight()||0,$(document).on("serviceforbidden",function(){window.location=context.root_url+"tao/Main/logout"}),serviceApi=eval(config.serviceApi),serviceApi.onFinish(function(){
$.ajax({url:config.finishDeliveryExecution,data:{deliveryExecution:config.deliveryExecution},type:"post",dataType:"json",success:function(data){window.location=data.destination}})}),$(document).on("loading",function(e){loadingBar.start()}).on("unloading",function(){setTimeout(function(){loadingBar.stop()},300)}).on("shutdown-com",function(){$(document).off("heightchange"),$frame.off("load.eventHeight").off("load.cors")}),serviceApi.loadInto($frame.get(0)),$(window).bind("resize",function(){resizeMainFrame()}),resizeMainFrame()}),function(window){"use strict";var appConfig=document.getElementById("amd-loader").getAttribute("data-config");require([appConfig],function(){
require(["taoDelivery/controller/runtime/deliveryExecution"])})}(window),define("taoDelivery/deliveryExecution",function(){});
//# sourceMappingURL=deliveryExecution.min.js.map