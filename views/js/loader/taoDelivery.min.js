define("taoDelivery/controller/DeliveryServer/index",["jquery","lodash","module","core/router","ui/feedback","layout/loading-bar"],function($,_,module,router,feedback,loadingBar){"use strict";return{start:function(parameters){var deliveryStarted=!1,config=module.config();parameters&&parameters.messages&&_.forEach(parameters.messages,function(message){!function(level,content){level&&content&&feedback($(".permanent-feedback"))[level](content,{timeout:-1,popup:!1})}(message.level,message.content)}),$("a.entry-point").on("click",function(e){var $elt=$(this);e.preventDefault(),e.stopPropagation(),deliveryStarted||$elt.hasClass("disabled")||function(url){
_.isString(url)&&!_.isEmpty(url)&&(deliveryStarted=!0,loadingBar.start(),window.location.href=url)}($elt.data().launch_url)}),config&&_.isArray(config.extraRoutes)&&config.extraRoutes.length&&router.dispatch(config.extraRoutes)}}}),define("taoDelivery/controller/routes",[],function(){"use strict";return{}}),define("tpl!taoDelivery/tpl/fullscreen-modal-feedback",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,options,buffer="",functionType="function",escapeExpression=this.escapeExpression,
