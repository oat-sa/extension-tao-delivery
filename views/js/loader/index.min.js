...
return compile(input,options,hb)},hb.precompile=function(input,options){return precompile(input,options,hb)},hb.AST=AST,hb.Compiler=Compiler,hb.JavaScriptCompiler=JavaScriptCompiler,hb.Parser=Parser,hb.parse=parse,hb};return Handlebars=create(),Handlebars.create=create,__exports__=Handlebars}(__module1__,__module7__,__module8__,__module10__,__module11__);return __module0__}();return Handlebars}),define("context",["module"],function(module){return module.config()}),define("i18n",["lodash","json!i18ntr/messages.json","context","core/format"],function(_,i18nTr,context,format){"use strict";var translations=i18nTr.translations,__=function(message){var localized=translations[message]?translations[message]:message;
return arguments.length>1&&(arguments[0]=localized,localized=format.apply(null,arguments)),localized};return __}),define("tpl",["handlebars","i18n","lodash"],function(hb,__,_){var buildMap={},extension=".tpl";return hb.registerHelper("__",function(key){return __(key)}),hb.registerHelper("join",function(arr,keyValueGlue,fragmentGlue,wrapper){var fragments=[];return keyValueGlue="string"==typeof keyValueGlue?keyValueGlue:void 0,fragmentGlue="string"==typeof fragmentGlue?fragmentGlue:" ",wrapper="string"==typeof wrapper?wrapper:'"',_.forIn(arr,function(value,key){var fragment="";null!==value||void 0!==value?"boolean"==typeof value?value=value?"true":"false":"object"==typeof value&&(value=_.values(value).join(" ")):value="",
...
config.isBuild?(buildMap[name]=fs.readFileSync(req.toUrl(name+extension)).toString(),onload()):req(["text!"+name+extension],function(raw){onload(hb.compile(raw))})},write:function(pluginName,moduleName,write){if(moduleName in buildMap){var compiled=hb.precompile(buildMap[moduleName]);write("define('tpl!"+moduleName+"', ['handlebars'], function(hb){ \nreturn hb.template("+compiled.toString()+");\n});\n")}}}}),define("tpl!ui/component/tpl/component",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<div class="component"></div>';
})}),define("ui/component",["jquery","lodash","core/eventifier","tpl!ui/component/tpl/component"],function($,_,eventifier,defaultTpl){"use strict";function delegatedResize(){var width=this.config.width,height=this.config.height,$container=this.getContainer(),$element=this.getElement();$container&&("auto"===width&&(width=$container.width()),"auto"===height&&(height=$container.height())),$element&&(_.isNumber(width)&&$element.css({width:width+"px"}),_.isNumber(height)&&$element.css({height:height+"px"})),this._width=width,this._height=height}function component(specs,defaults){var $container,componentTpl=defaultTpl,componentState={},componentApi={init:function(config){
return this.config=_(config||{}).omit(function(value){return null===value||"undefined"==typeof value}).defaults(defaults||{}).value(),componentState={},this.trigger("init"),this.config.renderTo&&($container=$(this.config.renderTo),this.render()),this},destroy:function(){return this.trigger("destroy"),this.$component&&this.$component.remove(),this.$component=null,componentState={},this},render:function(container){return container&&($container=$(container)),this.$component=$(componentTpl(this.config)),$container&&(this.config.replace&&$container.empty(),$container.append(this.$component)),this.setState("rendered",!0),delegatedResize.call(this),this.trigger("render",this.$component),
this},setSize:function(width,height){return this.config.width=width,this.config.height=height,this.is("rendered")&&delegatedResize.call(this),this.trigger("setsize",width,height),this},getSize:function(){if(this.is("rendered"))return{width:this._width||0,height:this._height||0}},show:function(){return this.trigger("show",this),this.setState("hidden",!1)},hide:function(){return this.trigger("hide",this),this.setState("hidden",!0)},enable:function(){return this.trigger("enable",this),this.setState("disabled",!1)},disable:function(){return this.trigger("disable",this),this.setState("disabled",!0)},is:function(state){return!!componentState[state]},setState:function(state,flag){
return flag=!!flag,componentState[state]=flag,this.$component&&this.$component.toggleClass(state,flag),this.trigger("state",state,flag,this),this},getContainer:function(){return $container},getElement:function(){return this.$component},getTemplate:function(){return componentTpl},setTemplate:function(template){var tpl=template||defaultTpl;return componentTpl=tpl,_.isFunction(componentTpl)||(componentTpl=function(){return tpl}),this.trigger("template",componentTpl),this}};return specs&&_(specs).functions().forEach(function(method){componentApi[method]=function(){return specs[method].apply(componentApi,_slice.call(arguments))}}),eventifier(componentApi)}var _slice=[].slice;
return component}),define("util/regexEscape",[],function(){"use strict";return function(s){return s.replace(/[-\/\\^$*+?.()|[\]{}]/g,"\\$&")}}),define("util/wrapLongWords",["util/regexEscape"],function(regexEscape){"use strict";function wrapLongWords(str,threshold){str=str.toString().replace(/([\w])</g,"$1 <");for(var cut,chunkExp=new RegExp(".{1,"+threshold+"}","g"),longWords=str.match(new RegExp("[\\S]{"+threshold+",}","g"))||[],i=longWords.length;i--;)cut=longWords[i].match(chunkExp).join(" "),str=str.replace(new RegExp(regexEscape(longWords[i]),"g"),cut);return str}return wrapLongWords}),define("util/encode",[],function(){"use strict";function encodeBase64(str){
...
$loadingBar.addClass("loading"),$loadingBar.toggleClass("loadingbar-covered",covered),updateHeight()},stop:function(){$loadingBar.removeClass("loading fixed").height(originalHeight)}}}),define("taoDelivery/controller/DeliveryServer/index",["jquery","lodash","ui/feedback","layout/loading-bar"],function($,_,feedback,loadingBar){"use strict";var displayPermanentMessage=function(level,content){level&&content&&feedback($(".permanent-feedback"))[level](content,{timeout:-1,popup:!1})};return{start:function(parameters){var deliveryStarted=!1,runDelivery=function(url){_.isString(url)&&!_.isEmpty(url)&&(deliveryStarted=!0,loadingBar.start(),window.location.href=url)};parameters&&parameters.message&&displayPermanentMessage(parameters.message.level,parameters.message.content),
$("a.entry-point").on("click",function(e){var $elt=$(this);e.preventDefault(),e.stopPropagation(),deliveryStarted||$elt.hasClass("disabled")||runDelivery($elt.attr("href"))})}}}),function(){"use strict";var loaderScript=document.getElementById("amd-loader"),configUrl=loaderScript.getAttribute("data-config"),bundle=loaderScript.getAttribute("data-bundle"),loadController=function(){var controllerOptions={},controllerPath=loaderScript.getAttribute("data-controller"),params=loaderScript.getAttribute("data-params");try{controllerOptions=JSON.parse(params)}catch(e){}require([controllerPath],function(controller){controller.start(controllerOptions)})};require([configUrl],function(){
bundle?require([bundle],loadController):loadController()})}(),define("loader/bootstrap",function(){});
