...
stack1=helpers["if"].call(depth0,depth0&&depth0.fsSupported,{hash:{},inverse:self.program(5,program5,data),fn:self.program(3,program3,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n        </div>\n    </div>\n</div>"})}),define("taoDelivery/controller/runtime/fullScreen",["jquery","i18n","ui/feedback","ui/modal","tpl!taoDelivery/tpl/fullscreen-modal-feedback"],function($,__,feedback,modal,dialogTpl){"use strict";var $dialog,$body,d=document,dElem=d.documentElement,fs=function(){var _fs={isSupported:!!(d.exitFullscreen||d.msExitFullscreen||d.mozCancelFullScreen||d.webkitExitFullscreen),requestFullscreen:dElem.requestFullscreen||dElem.msRequestFullscreen||dElem.mozRequestFullScreen||function(){
dElem.webkitRequestFullscreen(Element.ALLOW_KEYBOARD_INPUT)},fullscreenchange:function(){for(var prefixes=["","ms","moz","webkit"],i=prefixes.length;i--;)if("on"+prefixes[i]+"fullscreenchange"in dElem)return prefixes[i]+"fullscreenchange";return"myfullscreenchange"}(),fullScreen:function(){return document.fullscreenElement&&null!==document.fullscreenElement||document.mozFullScreen||document.webkitIsFullScreen||(screen.availHeight||screen.height-30)<=window.innerHeight}};return _fs.changeInterval=null,_fs.awaitFsChange=function(){var event=document.createEvent("Event");event.initEvent(_fs.fullscreenchange,!0,!0),_fs.changeInterval=setInterval(function(){_fs.fullScreen()||document.dispatchEvent(event);
},2e3)},_fs.isSupported||_fs.awaitFsChange(),_fs}(),handleUserInput=function(evt){return fs.isSupported?void(("keydown"!==evt.type||13===(evt.keyCode||evt.which))&&(fs.requestFullscreen.call(dElem),dElem.className+=" fullscreen",$dialog.modal("close"))):void $dialog.modal("close")},init=function(){$body=$(document.body),document.addEventListener(fs.fullscreenchange,function(){fs.fullScreen()||(dElem.className=dElem.className.replace(/\bfullscreen\b/,""),$dialog.modal("open"))}),modal($body),$dialog=$(dialogTpl({fsSupported:fs.isSupported,launchButton:0===navigator.platform.toLowerCase().indexOf("Mac")?"Ctrl+⌘+F":"F11"})),$dialog[0].querySelector("button").addEventListener("click",function(e){
handleUserInput(e)}),document.addEventListener("keydown",function(e){fs.fullScreen()||handleUserInput(e)}),$dialog.on("opened.modal",function(){clearInterval(fs.changeInterval)}),$dialog.on("closed.modal",function(){fs.awaitFsChange()}),$body.append($dialog),$dialog.modal({width:500,disableClosing:!0,startClosed:!0}),!1==(screen.availHeight||screen.height-30)<=window.innerHeight&&$dialog.modal("open")};return{init:init}}),define("taoDelivery/controller/runtime/deliveryExecution",["lodash","jquery","helpers","taoDelivery/controller/runtime/fullScreen","layout/loading-bar"],function(_,$,helpers,fullScreen,loadingBar){"use strict";function resizeMainFrame(){var height=$(window).outerHeight()-$headerHeight-$footerHeight;
$frameContainer.height(height),$frame.height(height)}var $frameContainer,$frame,$headerHeight,$footerHeight;return{start:function(options){options.deliveryServerConfig.requireFullScreen&&fullScreen.init(),$frameContainer=$("#outer-delivery-iframe-container"),$frame=$frameContainer.find("iframe"),$headerHeight=$("body > .content-wrap > header").outerHeight()||0,$footerHeight=$("body > footer").outerHeight()||0,$(document).on("serviceforbidden",function(){window.location=helpers._url("logout","Main","tao")});var serviceApi=options.serviceApi;serviceApi.onFinish(function(){$.ajax({url:options.finishDeliveryExecution,data:{deliveryExecution:options.deliveryExecution
