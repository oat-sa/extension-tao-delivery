...
})}}}),define("taoDelivery/controller/deliveryAssembly",["jquery","i18n","helpers","ui/feedback","module"],function($,__,helpers,feedback,module){return{start:function(){var conf=module.config();conf.message&&feedback().info(conf.message),$("#saver-status").click(function(){var status=$('input[name="status"]:checked').val(),uri=$('input[name="assemblyUri"]').val();$.ajax({url:helpers._url("setStatus","Delivery","taoDelivery"),type:"POST",dataType:"text",data:{uri:uri,status:status},success:function(){feedback().success(__("Status saved")),$(".tree").trigger("refresh.taotree")},error:function(jqXHR,textStatus,errorThrown){feedback().error("Error occured: "+errorThrown)
