...
},init=function(){$body=$(document.body),document.addEventListener(fs.fullscreenchange,function(){fs.fullScreen()||(dElem.className=dElem.className.replace(/\bfullscreen\b/,""),$dialog.modal("open"))}),modal($body),$dialog=$(dialogTpl({fsSupported:fs.isSupported,launchButton:0===navigator.platform.toLowerCase().indexOf("Mac")?"Ctrl+⌘+F":"F11"})),$dialog[0].querySelector("button").addEventListener("click",function(e){handleUserInput(e)}),document.addEventListener("keydown",function(e){fs.fullScreen()||handleUserInput(e)}),$dialog.on("opened.modal",function(){clearInterval(fs.changeInterval)}),$dialog.on("closed.modal",function(){fs.awaitFsChange()}),$body.append($dialog),$dialog.modal({width:500,disableClosing:!0,startClosed:!0}),!1==(screen.availHeight||screen.height-30)<=window.innerHeight&&$dialog.modal("open")
};return{init:init}}),define("taoDelivery/controller/runtime/deliveryExecution",["jquery","iframeResizer","context","taoDelivery/controller/runtime/fullScreen","layout/loading-bar"],function($,iframeResizer,context,fullScreen,loadingBar){function resizeMainFrame(){var height=$(window).outerHeight()-$headerHeight-$footerHeight;$frameContainer.height(height),$frame.height(height)}var $frameContainer,$frame,$headerHeight,$footerHeight;return{start:function(options){options.deliveryServerConfig.requireFullScreen&&fullScreen.init(),$frameContainer=$("#outer-delivery-iframe-container"),$frame=$frameContainer.find("iframe"),$headerHeight=$("body > .content-wrap > header").outerHeight()||0,$footerHeight=$("body > footer").outerHeight()||0,$(document).on("serviceforbidden",function(){window.location=context.root_url+"tao/Main/logout"
