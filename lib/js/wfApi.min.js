...
var wfApi={context:window.top.document||window.document};function forward(){var next=wfApi.context.getElementById('next');if(next){}}
...
if(typeof(finish)!='function'){var _stateContext=window;function finish(){$(_stateContext).trigger(wf_STATE.ITEM.PRE_FINISHED);$(_stateContext).trigger(wf_STATE.ITEM.FINISHED);$(_stateContext).trigger(wf_STATE.ITEM.POST_FINISHED);}
function onFinish(callback){$(_stateContext).bind(wf_STATE.ITEM.FINISHED,callback);}
function beforeFinish(callback){$(_stateContext).bind(wf_STATE.ITEM.PRE_FINISHED,callback);}
function afterFinish(callback){$(_stateContext).bind(wf_STATE.ITEM.POST_FINISHED,callback);}}
