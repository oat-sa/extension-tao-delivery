...
if(environment.method){if(/^get|post$/i.test(environment.method)){this.sourceService.method=environment.method;}}}}}};this.retrieveContext=function(){if(key!=''){switch(this.sourceService.type){case'manual':this.registry=this.sourceService.data;break;case'sync':try{var response=$.parseJSON($.ajax({async:false,url:this.sourceService.url,data:this.sourceService.params,type:this.sourceService.method,dataType:this.sourceService.format}).responseText);if($.isPlainObject(response)||$.isArray(response)){this.registry=response;}}
catch(jsonException){}
...
$.ajax({async:(this.destinationService.type=='async'),url:this.destinationService.url,data:registryParams,type:this.destinationService.method,dataType:this.destinationService.format,success:function(data){if(data.saved){if(_recoveryCtx.destinationService.flush){_recoveryCtx.registry=new Object();}}}});};this.getContext=function(key){if(this.registry==null){this.retrieveContext(key);}
